"""Host-side C runtime round-trip test against existing cloud /infer/split API."""
...
def _can_connect_loopback() -> bool:
    """Return True when loopback TCP connect is permitted in this environment."""
    try:
...
def test_c_cli_roundtrip_to_cloud(
    c_runtime_build_dir: Path,
    edge_k7_c_artifacts: Path,
    repo_root: Path,
):
    ...
    try:
        ...
        cli_bin = c_runtime_build_dir / "unisplit_edge_k7_cli"
        run = subprocess.run(
            [
                str(cli_bin),
                "--artifacts-dir",
                str(edge_k7_c_artifacts),
                "--post",
                "--cloud-url",
                base_url,
                "--model-version",
        ...
        assert run.returncode == 0, f"C CLI failed:\nstdout:\n{run.stdout}\nstderr:\n{run.stderr}"
        assert "CLOUD_OK" in run.stdout
        assert "status=ok" in run.stdout

        m = re.search(r"class=(\d+)", run.stdout)
        assert m, f"Could not parse predicted class from output:\n{run.stdout}"
        cls = int(m.group(1))
        assert 0 <= cls < 34
