...
FROZEN_ROOT="${ROOT_DIR}/artifacts/pi_phase2/frozen-hwpass-20260408"
SOURCE_TREE="${FROZEN_ROOT}/source_tree/unikraft"
APP_DIR="${ROOT_DIR}/edge_native/unikraft_pi4_phase3_selftest"
UK_DIR="${APP_DIR}/.unikraft/unikraft"
BUILD_DIR="${APP_DIR}/.unikraft/build"
APP_CONFIG="${APP_DIR}/.config.pi4-phase3"
DEFCONFIG="${ROOT_DIR}/configs/pi_phase3/unisplit_pi4_phase3_selftest.defconfig"
PATCH_FILE="${ROOT_DIR}/configs/patches/unikraft_pi4_phase3_selftest.patch"
ARTIFACT_ROOT="${ROOT_DIR}/artifacts/pi_phase3/latest"
IMAGES_DIR="${ARTIFACT_ROOT}/images"
AUDIT_FILE="${ARTIFACT_ROOT}/pi4_phase3_build_audit.txt"
...
usage() {
  cat <<'EOF' >&2
Usage: scripts/build_pi4_phase3.sh [--audit-only]
EOF
  exit 2
}
...
need_cmd() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "[pi4-phase3] Missing required tool: $1" >&2
    exit 1
  }
}
...
if [[ ! -d "${SOURCE_TREE}" ]]; then
  echo "[pi4-phase3] Missing frozen source snapshot: ${SOURCE_TREE}" >&2
  exit 1
fi

if [[ ! -f "${FROZEN_ROOT}/FROZEN_BASELINE.txt" ]]; then
  echo "[pi4-phase3] Missing frozen baseline metadata: ${FROZEN_ROOT}/FROZEN_BASELINE.txt" >&2
  exit 1
fi

if [[ ! -f "${PATCH_FILE}" ]]; then
  echo "[pi4-phase3] Missing Phase 3 patch file: ${PATCH_FILE}" >&2
  exit 1
fi
...
if ! patch -p1 --dry-run -d "${UK_DIR}" < "${PATCH_FILE}" >/dev/null 2>&1; then
  echo "[pi4-phase3] Could not apply Phase 3 patch cleanly in ${UK_DIR}: ${PATCH_FILE}" >&2
  exit 1
fi
...
echo "[pi4-phase3] Configuring Pi4 phase-3 build..."
make -C "${UK_DIR}" \
  A="${APP_DIR}" \
  O="${BUILD_DIR}" \
  C="${APP_CONFIG}" \
...
echo "[pi4-phase3] Building Pi4 phase-3 image..."
make -C "${UK_DIR}" \
  A="${APP_DIR}" \
  O="${BUILD_DIR}" \
  C="${APP_CONFIG}" \
...
if [[ ! -f "${CFG_FILE}" ]]; then
  echo "[pi4-phase3] Missing build config: ${CFG_FILE}" >&2
  exit 1
fi
...
required_cfg=(
  ...
  '^CONFIG_APPPI4PHASE3_SELFTEST=y$'
)

for pattern in "${required_cfg[@]}"; do
  if ! rg -n "${pattern}" "${CFG_FILE}" >/dev/null; then
    echo "[pi4-phase3] Missing required config: ${pattern}" >&2
    exit 1
  fi
done
...
for pattern in "${rejected_cfg[@]}"; do
  if rg -n "${pattern}" "${CFG_FILE}" >/dev/null; then
    echo "[pi4-phase3] Rejected config present: ${pattern}" >&2
    exit 1
  fi
done
...
if [[ -z "${RPI4_CMD}" ]]; then
  echo "[pi4-phase3] Missing Pi4 page-table build command in ${BUILD_DIR}" >&2
  exit 1
fi

if find "${BUILD_DIR}" -type f -name 'qemu_bpt64.arm.o.cmd' | grep -q .; then
  echo "[pi4-phase3] QEMU page-table object detected in Pi4 build output." >&2
  exit 1
fi

if find "${BUILD_DIR}" -maxdepth 1 -type f \( -name '*.efi' -o -name '*BOOTAA64*' \) | grep -q .; then
  echo "[pi4-phase3] EFI artifacts detected in direct-boot build output." >&2
  exit 1
fi
...
if [[ -z "${IMG_PATH}" ]]; then
  echo "[pi4-phase3] Missing Pi4 base image (*_kvm-arm64) in ${BUILD_DIR}" >&2
  exit 1
fi

if [[ -z "${IMG_CMD_PATH}" || -z "${BIN_CMD_PATH}" ]]; then
  echo "[pi4-phase3] Missing Linux direct-boot command traces (*.bin.cmd / *.img.cmd)." >&2
  exit 1
fi
...
required_markers=(
  ...
  'UNISPLIT_RPI4_P3_SELFTEST_FAIL:'
)

for marker in "${required_markers[@]}"; do
  if ! strings "${IMG_PATH}" | grep -F "${marker}" >/dev/null; then
    echo "[pi4-phase3] Missing required marker ${marker} in ${IMG_PATH}" >&2
    exit 1
  fi
done
...

{
  ...
  echo "phase2_frozen_baseline=${FROZEN_ROOT}/FROZEN_BASELINE.txt"
  echo "markers=UNISPLIT_RPI4_P1_ASM_ENTRY,UNISPLIT_RPI4_P1_POST_MMU,UNISPLIT_RPI4_P2_PRE_BOOTINFO_CALL,UNISPLIT_RPI4_P2_POST_BOOTINFO,UNISPLIT_RPI4_P2_UKPLAT_ENTRY,UNISPLIT_RPI4_P3_MAIN_ENTRY,UNISPLIT_RPI4_P3_SELFTEST_START,UNISPLIT_RPI4_P3_SELFTEST_PASS,UNISPLIT_RPI4_P3_SELFTEST_DONE,UNISPLIT_RPI4_P3_SELFTEST_FAIL:"
} > "${AUDIT_FILE}"
...
if [[ "${MODE}" == "build" ]]; then
  ...
  echo "[ok] Pi4 phase-3 build completed."
  echo "[img ] ${IMAGES_DIR}/kernel8.img"
else
  echo "[ok] Pi4 phase-3 audit completed."
fi
