...
IMAGE_PATH="${ROOT_DIR}/artifacts/pi_phase1/latest/images/kernel8.img"
OUT_DIR="${ROOT_DIR}/artifacts/pi_phase1/latest/sdcard"
CONFIG_SRC="${ROOT_DIR}/configs/pi_boot/config.txt"
MANIFEST_FILE="${OUT_DIR}/pi4_phase1_sdcard_manifest.txt"
README_FILE="${OUT_DIR}/README_PHASE1.txt"
SHA_FILE="${OUT_DIR}/SHA256SUMS.txt"
TMP_DIR="${ROOT_DIR}/artifacts/pi_phase1/latest/.stage_tmp"
BUNDLE_PATH="${1:-${UNISPLIT_PI_FIRMWARE_BUNDLE:-${UNISPLIT_PI_UEFI_BUNDLE:-}}}"

need_cmd() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "[pi4-phase1-stage] Missing required tool: $1" >&2
    exit 1
  }
}
...
if [[ -z "${BUNDLE_PATH}" ]]; then
  echo "[pi4-phase1-stage] Missing firmware bundle path." >&2
  echo "[pi4-phase1-stage] Set UNISPLIT_PI_FIRMWARE_BUNDLE=/abs/path/RPi4_UEFI_Firmware_v1.51.zip" >&2
  exit 1
fi

if [[ ! -f "${BUNDLE_PATH}" ]]; then
  echo "[pi4-phase1-stage] Firmware bundle not found: ${BUNDLE_PATH}" >&2
  exit 1
fi

if [[ ! -f "${IMAGE_PATH}" ]]; then
  echo "[pi4-phase1-stage] Missing built kernel image: ${IMAGE_PATH}" >&2
  exit 1
fi
...
if find "${OUT_DIR}" -maxdepth 2 -type f \( -name 'RPI_EFI.fd' -o -name 'BOOTAA64.EFI' -o -name 'cmdline.txt' \) | grep -q .; then
  echo "[pi4-phase1-stage] Forbidden UEFI file detected in direct-boot SD tree." >&2
  exit 1
fi

cat > "${README_FILE}" <<'EOF'
UniSplit Raspberry Pi 4 Phase-1 SD Card Tree
===========================================

This directory is the direct-boot proof-of-life SD layout.
...
echo "[ok] Pi4 phase-1 SD card tree prepared."
echo "[dir ] ${OUT_DIR}"
